...
<div class="content-wrapper">
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Home/</span> Menu Settings</h4>
        ...
        <div class="col-xxl">
          <div class="card mb-4">
            <div class="card-header d-flex align-items-center justify-content-between">
              <h5 class="mb-0">Header Main Menu</h5>
              <small class="text-muted float-end">Main Menu Content</small>
            </div>
            <div class="card-body img-item-sec">
              <div class="row mb-5">
                <form class="form-inline" id="menu-add">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="addInputName" placeholder="menu name" required>
                  </div>
                  <div>&nbsp;</div>
                  <div class="form-group">
                    <label for="addInputSlug">URL Link &nbsp;</label>
                    <input type="text" class="form-control" id="addInputSlug" placeholder="#" required>
                  </div>
                  <div>&nbsp;</div>
              ...
              <div class="row">
                <h4 class="title">Main Menu Items</h4>
                <form method="post" action="{{ route('showMenu') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="dd nestable" id="nestable">
                        <ol class="dd-list">
                            @foreach($menuItems as $menuItem)
                                <!-- Main Menu Item -->
                                <li class="dd-item" data-id="{{ $menuItem->id }}" data-name="{{ $menuItem->name }}" data-slug="{{ $menuItem->slug }}" data-new="0" data-deleted="0">
                                    <div class="dd-handle">{{ $menuItem->name }}</div>
                                    <span class="button-delete btn btn-danger btn-xs pull-right" data-owner-id="{{ $menuItem->id }}">
                                        <i class="bx bxs-message-square-x"></i>
                                    </span>
                                    <span class="button-edit btn btn-success btn-xs pull-right" data-owner-id="{{ $menuItem->id }}">
                                        <i class="bx bx-edit" aria-hidden="true"></i>
                                    </span>

                                    <!-- Check if the menu item has children -->
                                      @if($menuItem->children ? $menuItem->children->count() : 0 > 0)
                                        <ol class="dd-list">
                                            <!-- Loop through the sub-menu items -->
                                            @foreach($menuItem->children as $subMenuItem)
                                                <li class="dd-item" data-id="{{ $subMenuItem->id }}" data-name="{{ $subMenuItem->name }}" data-slug="{{ $subMenuItem->slug }}" data-new="0" data-deleted="0">
                                                    <div class="dd-handle">{{ $subMenuItem->name }}</div>
                                                    <span class="button-delete btn btn-danger btn-xs pull-right" data-owner-id="{{ $subMenuItem->id }}">
                                                        <i class="bx bxs-message-square-x"></i>
                                                    </span>
                                                    <span class="button-edit btn btn-success btn-xs pull-right" data-owner-id="{{ $subMenuItem->id }}">
                                                        <i class="bx bx-edit" aria-hidden="true"></i>
                                                    </span>
...
var addToMenu = function () {
  ...
  nestableList.append(
    ...
    '<i class="bx bx-edit" aria-hidden="true"></i>' +
    '</span>' +
    '<input type="hidden" name="newMenuItems[' + newId + '][data-id]" value="' + newId + '">' +
    '<input type="hidden" name="newMenuItems[' + newId + '][data-name]" value="' + newName + '">' +
    '<input type="hidden" name="newMenuItems[' + newId + '][data-slug]" value="' + newSlug + '">' +
    '</li>'
  );
...
;(function($, window, document, undefined)
{
    ...
    Plugin.prototype = {
        ...
        dragStop: function (e) {
          ...
          this.updateParentChildRelationship();

          // update JSON
          updateOutput($('#nestable').data('output', $('#json-output')));
      },

      updateParentChildRelationship: function () {
            var list = this.el;
            var self = this;

            list.find(self.options.itemNodeName).each(function () {
                var item = $(this);
                var parentId = null;

                // Check if the item has a parent
                if (item.closest(self.options.listNodeName).length > 0) {
                    parentId = item.closest(self.options.itemNodeName).data('id');
                }

                var children = item.children(self.options.listNodeName);

                if (children.length > 0) {
                    children.children(self.options.itemNodeName).each(function () {
                        var child = $(this);
                        var childId = child.data('id');

                        // Update the parent_id field for each child
                        child.find('input[name^="newMenuItems"]').each(function () {
                            var input = $(this);
                            input.attr('name', input.attr('name').replace('newMenuItems', 'newMenuItems[' + parentId + '][children][' + childId + ']'));
                        });
                    });
                }
            });
